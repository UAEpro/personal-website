...
Manage blog posts on uaepro.me through its REST API. Create draft posts, publish them, manage categories/tags, and list existing content — all from Claude Code.

## Configuration

**API URL:** `__SITE_URL__`
**API Key:** `__API_KEY__`

All API calls use this base URL and Bearer token. Use the Bash tool with `curl` for all requests:
...
|--------|--------|----------|------|
| List | GET | `__SITE_URL__/api/posts?status=DRAFT&page=1&limit=10` | — |
| Get by ID | GET | `__SITE_URL__/api/posts/{id}` | — |
| Create | POST | `__SITE_URL__/api/posts` | `{title, content, excerpt, status, categoryId, tags}` |
| Update | PUT | `__SITE_URL__/api/posts/{id}` | partial fields |
| Delete | DELETE | `__SITE_URL__/api/posts/{id}` | — |
...
| Action | Method | Endpoint | Body |
|--------|--------|----------|------|
| List categories | GET | `__SITE_URL__/api/categories` | — |
| Create category | POST | `__SITE_URL__/api/categories` | `{name}` |
| List tags | GET | `__SITE_URL__/api/tags` | — |
| Create tag | POST | `__SITE_URL__/api/tags` | `{name}` |
...
```bash
curl -s -X POST "__SITE_URL__/api/posts" \
...
```bash
curl -s -X PUT "__SITE_URL__/api/posts/{id}" \
  -H "Authorization: Bearer __API_KEY__" \
  -H "Content-Type: application/json" \
  -d '{"status": "PUBLISHED"}'
```
...
```bash
# List recent drafts
curl -s "__SITE_URL__/api/posts?status=DRAFT&limit=10" \
  -H "Authorization: Bearer __API_KEY__"

# List published posts
curl -s "__SITE_URL__/api/posts?status=PUBLISHED&limit=10" \
  -H "Authorization: Bearer __API_KEY__"

# Search posts
curl -s "__SITE_URL__/api/posts?search=keyword" \
  -H "Authorization: Bearer __API_KEY__"
```
